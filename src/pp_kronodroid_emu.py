...
if __name__=="__main__":
    ...
    dataset0.drop(columns = dif, inplace = True)

    dataset = pd.concat([dataset0, dataset1]).reset_index(drop = True)
    dataset.to_csv('kronodroid_emulator.csv', index = False)
    ...
    cast_to_numeric(dataset, exclude = ['Package', 'sha256', 'FirstModDate', 'LastModDate'])
    ...
    to_drop_columns = ['Package', 'sha256', 'CFileSize', 'UFileSize', 'FirstModDate', 'LastModDate', 'TimesSubmitted', 'Scanners']
